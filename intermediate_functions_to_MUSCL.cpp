...
std::tuple<double,double> speedsx( const std::array<double , 9>&x , double gamma){
...
std::tuple<double,double,double> wavespeedx(const std::array<double,9>&x,const std::array<double,9>&y,double gamma, double ch){
    
    auto[cs2L,cfa2L] = speedsx(x,gamma);
    auto[cs2R,cfa2R] = speedsx(y,gamma);
...
std::tuple<double,double> speedsy( const std::array<double , 9>&x , double gamma ){
...
std::tuple<double,double,double> wavespeedy(const std::array<double,9>x,const std::array<double,9>y,double gamma, double ch){
   
    auto[cs2L,cfa2L] = speedsy(x,gamma);
    auto[cs2R,cfa2R] = speedsy(y,gamma);
    auto primL = ConservativeToPrimitive(x,gamma);
    auto primR = ConservativeToPrimitive(y,gamma);
...
std::array<double , 9> uHLLx ( const std::array<double , 9>&x , const std::array<double , 9>& y , double gamma,double ch){
    
    auto [sL, qStar, sR] = wavespeedx(x, y, gamma, ch);
...
std::array<double , 9> uHLLy ( const std::array<double , 9>&x , const std::array<double , 9>& y , double gamma, double ch){
   
    
    auto [sL, qStar, sR] = wavespeedy(x, y, gamma, ch);
...
std::tuple<std::array<double , 9>, std::array<double , 9>> uStarX( const std::array<double , 9>&x , const std::array<double , 9>& y , double gamma , double ch){
    auto [BxTilde, psiTilde] = psiAndBx(x,y,gamma,ch);

    auto [sL,qStar,sR] = wavespeedx(x,y,gamma,ch);
    auto primL = ConservativeToPrimitive(x,gamma);
    auto primR = ConservativeToPrimitive(y,gamma);
    auto uHLL = uHLLx(x,y,gamma,ch);
    double rhoL = x[0], u_xL = primL[1],u_yL = primL[2],u_zL = primL[3], EL = x[4], pL = primL[4], BxL = BxTilde, ByL=x[6], BzL=x[7];
    double rhoR = y[0], u_xR = primR[1],u_yR = primR[2],u_zR = primR[3], ER = y[4], pR = primR[4], BxR = BxTilde, ByR=y[6], BzR=y[7];
    ...
    double vDotBHLL = BxHLL*u_xHLL + ByHLL * u_yHLL + BzHLL * u_zHLL;

    double rhoStarL = rhoL*(sL - u_xL)/(sL - qStar);
    double energyStarL = EL*(rhoStarL/rhoL) + ((pStarL*qStar - pTL*u_xL) - (BxStar*vDotBHLL - BxL*vDotBL))/(sL - qStar);

    std::array<double ,9> uStarL;
    uStarL[0] = rhoStarL;
    uStarL[1] = rhoStarL * qStar;
    uStarL[2] = u_yL*rhoStarL - (BxStar*ByHLL - BxL*ByL)/(sL - qStar);
    uStarL[3] = u_zL*rhoStarL - (BxStar*BzHLL - BxL*BzL)/(sL - qStar);
    ...
    double energyStarR = ER*(rhoStarR/rhoR) + ((pStarR*qStar - pTR*u_xR) - (BxStar*vDotBHLL - BxR*vDotBR))/(sR - qStar);


    std::array<double ,9> uStarR;
    uStarR[0] = rhoStarR;
    uStarR[1] = rhoStarR * qStar;
    uStarR[2] = u_yR*rhoStarR - (BxStar*ByHLL - BxR*ByR)/(sR - qStar);
    uStarR[3] = u_zR*rhoStarR - (BxStar*BzHLL - BxR*BzR)/(sR - qStar);
...
std::tuple<std::array<double , 9>, std::array<double , 9>> uStarY( const std::array<double , 9>&x , const std::array<double , 9>& y , double gamma , double ch){
    auto [ByTilde , psiTilde] = psiAndBy(x,y,gamma,ch);
   
    auto [sL,qStar,sR] = wavespeedy(x,y,gamma,ch);
    auto primL = ConservativeToPrimitive(x,gamma);
    auto primR = ConservativeToPrimitive(y,gamma);
    auto uHLL = uHLLy(x,y,gamma,ch);
    double rhoL = x[0], u_xL = primL[1],u_yL = primL[2],u_zL = primL[3], EL = x[4], pL = primL[4], BxL = x[5], ByL=ByTilde, BzL=x[7];
    double rhoR = y[0], u_xR = primR[1],u_yR = primR[2],u_zR = primR[3], ER = y[4], pR = primR[4], BxR = y[5], ByR=ByTilde, BzR=y[7];
    double u_xHLL = uHLL[1], u_yHLL = uHLL[2], u_zHLL = uHLL[3], BxHLL=uHLL[5], ByHLL = uHLL[6], BzHLL = uHLL[7];
    double B2L = BxL*BxL + ByL*ByL + BzL*BzL;
    double B2R = BxR*BxR + ByR*ByR + BzR*BzR;
    double pTL = pL + 0.5*B2L;
    ...
    double energyStarL = EL*(rhoStarL/rhoL) + ((pStarL*qStar - pTL*u_yL) - (ByStar*vDotBHLL - ByL*vDotBL))/(sL - qStar);

    std::array<double ,9> uStarL;
    uStarL[0] = rhoStarL;
    uStarL[1] = rhoStarL*u_xL - (BxHLL*ByStar - BxL*ByL)/(sL - qStar);
    uStarL[2] = rhoStarL*qStar;
    uStarL[3] = rhoStarL*u_zL - (ByStar*BzHLL - ByL*BzL)/(sL - qStar);
    uStarL[4] = energyStarL;
    uStarL[5] = BxHLL;
    uStarL[6] = ByStar;
    uStarL[7] = BzHLL;
    ...
    double energyStarR = ER*(rhoStarR/rhoR) + ((pStarR*qStar - pTR*u_yR) - (ByStar*vDotBHLL - ByR*vDotBR))/(sR - qStar);

    std::array<double ,9> uStarR;
    uStarR[0] = rhoStarR;
    uStarR[1] = rhoStarR*u_xR - (BxHLL*ByStar - BxR*ByR)/(sR - qStar);
    uStarR[2] = rhoStarR*qStar;
    uStarR[3] = rhoStarR*u_zR - (ByStar*BzHLL - ByR*BzR)/(sR - qStar);
    uStarR[4] = energyStarR;
    uStarR[5] = BxHLL;
    uStarR[6] = ByStar;
    uStarR[7] = BzHLL;
...
std::array<double,9> FluxHLLCX(const std::array<double,9>& x, const std::array<double,9>& y, double gamma, double ch){
    auto [uStarL, uStarR] = uStarX(x,y,gamma,ch);//already div-cleaned
    auto [BxTilde, psiTilde] = psiAndBx(x,y,gamma,ch);
    std::array<double,9> x1,y1;
    x1=x;
    y1=y;
    x1[5] = y1[5] = BxTilde;//apply divergence cleaning
    x1[8] = y1[8] = psiTilde;
    auto [sL, qStar, sR] = wavespeedx(x1, y1, gamma, ch);
    std::array<double , 9> FluxL = FluxDefX(x1,gamma);
    std::array<double , 9> FluxR = FluxDefX(y1,gamma);
...
std::array<double,9> FluxHLLCY(const std::array<double,9> &x, const std::array<double,9> &y, double gamma, double ch){
    auto [uStarL, uStarR] = uStarY(x,y,gamma,ch);//already div-cleaned
    auto [ByTilde, psiTilde] = psiAndBy(x,y,gamma,ch);
    std::array<double,9> x1,y1;
    x1=x;
    y1=y;
    x1[6] = y1[6] = ByTilde;
    x1[8] = y1[8] = psiTilde;
    auto [sL, qStar, sR] = wavespeedy(x1, y1, gamma, ch);
    std::array<double , 9> FluxL = FluxDefY(x1,gamma);
    std::array<double , 9> FluxR = FluxDefY(y1,gamma);
    std::array<double , 9> FluxHLLC;
    if (sL >= 0) {
        FluxHLLC = FluxL;
    } else if (sL <= 0 && qStar >= 0) {
...
std::array<double , 9> FluxHLLX(const std::array<double , 9>&x , const std::array<double , 9> &y , double gamma,double ch){
    auto [uStarL, uStarR] = uStarX(x,y,gamma,ch);
    auto [BxTilde, psiTilde] = psiAndBx(x,y,gamma,ch);
    std::array<double,9> x1,y1;
    x1=x;
    y1=y;
    ...
    std::array<double , 9> Flux;
    if(sL >= 0){
        Flux = FluxDefX(x1,gamma);
    }
    else if(sL < 0 && sR > 0){
        auto FluxX = FluxDefX(x1,gamma);
        auto FluxY = FluxDefX(y1,gamma);
        for(int i=0 ; i<9; ++i){
            Flux[i] = (sR*FluxX[i] - sL*FluxY[i] + sL*sR*(y1[i] - x1[i]))/(sR - sL);
        }
    }
    else { // sR <= 0
        Flux = FluxDefX(y1,gamma);
    }
    Flux[5] = psiTilde;
...
std::array<double , 9> FluxHLLY(const std::array<double , 9>&x , const std::array<double , 9>& y , double gamma,double ch){
    auto [uStarL, uStarR] = uStarY(x,y,gamma,ch);
    auto [ByTilde, psiTilde] = psiAndBy(x,y,gamma,ch);
    std::array<double,9> x1,y1;
    x1=x;
    y1=y;
    ...
    std::array<double , 9> Flux;
    if(sL >= 0){
        Flux = FluxDefY(x1,gamma);
    }
    else if(sL < 0 && sR > 0){
        auto FluxX = FluxDefY(x1,gamma);
        auto FluxY = FluxDefY(y1,gamma);
        for(int i=0 ; i<9; ++i){
            Flux[i] = (sR*FluxX[i] - sL*FluxY[i] + sL*sR*(y1[i] - x1[i]))/(sR - sL);
        }
    }
    else { // sR <= 0
        Flux = FluxDefY(y1,gamma);
    }
    Flux[6] = psiTilde;
